...
use crate::{image::Image, voxel::{self, VoxelBlock}};
...
fn should_carve(x: f32, y: f32, z: f32, images: &mut Vec<Image>) -> bool {
    for image in images {
        if !is_consistent_with_image(x, y, z, image) {
            return true;
        }
    }
    false
...
fn is_consistent_with_image(x: f32, y: f32, z: f32, image: &mut Image) -> bool {
    // convert from world space to projected/clip space
    let proj_coord = image.camera.get_proj_matrix() * image.camera.get_view_matrix() * Vector4::new(x, y, z, 1.0);
    
    // normalize
    let normed_coord = Vector3::new(proj_coord[0] / proj_coord[3], proj_coord[1] / proj_coord[3], proj_coord[2] / proj_coord[3]);
